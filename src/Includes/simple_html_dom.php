    ...
    function dump($show_attr = true, $deep = 0)
    {
        ...
        if ($show_attr && count($this->attr) > 0) {
            ...
            foreach ($this->attr as $k => $v) {
                echo '[' . esc_html($k) . ']=>"' . esc_html($this->$k) . '", ';
            }
            echo ')';
        }
        echo "\n";
...
// This add next line is documented in the sourceforge repository. 2977248 as a fix for ongoing memory leaks that occur
even with the use of clear.
...
protected function prepare($str, $lowercase = true, $stripRN = true, $defaultBRText = DEFAULT_BR_TEXT, $defaultSpanText
= DEFAULT_SPAN_TEXT)
{
$this->clear();
...
{
if (($s = $this->copy_until_char('<'))==='' ) { return $this->read_tag();
    }
    ...
    // NOTE: IF you provide a routine called get_last_retrieve_url_contents_content_type which returns the
    ...
    // and if this doesn't work... then we need to just wrongheadedly assume it's UTF-8 so that we can move on - cause
    this will usually give us most of what we need...
    if ($charset === false) {
    if (is_object($debugObject)) {
                ...
                $this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
                    ...
                    $node->_[HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until_char('>');
                        ...
                        $this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
                            ...
                            // text
                            if ($pos = strpos($tag, '<') !==false) { $tag='<' . substr($tag, 0, -1); $node->
                                _[HDOM_INFO_TEXT] = $tag;
                                $this->link_nodes($node, false);
                                ...
                                $node->_[HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until('<>');
                                        if ($this->char === '<') { $this->link_nodes($node, false);
                                            return true;
                                            }
                                            ...
                                            $this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null;
                                                ...
                                                $this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] :
                                                    ...
                                                    // handle endless '<' if ($this->pos >= $this->size - 1 &&
                                                        $this->char !== '>') {
                                                        ...
                                                        $node->_[HDOM_INFO_TEXT] = '<' . $tag . $space[0] . $name;
                                                            ...
                                                            // handle mismatch '<' if ($this->doc[$this->pos - 1] == '
                                                                <') { $node->nodetype = HDOM_TYPE_TEXT;
                                                                    $node->tag = 'text';
                                                                    $node->attr = array();
                                                                    $node->_[HDOM_INFO_END] = 0;
                                                                    ...
                                                                    $this->char = (++$this->pos < $this->size) ?
                                                                        ...
                                                                        $this->char = (++$this->pos < $this->size) ?
                                                                            ...
                                                                            $this->char = (++$this->pos < $this->size) ?
                                                                                ...
                                                                                // If it's a BR tag, we need to set it's
                                                                                text to the default text.
                                                                                // This way when we see it in plaintext,
                                                                                we can generate formatting that the user
                                                                                wants.
                                                                                // since a br tag never has sub nodes,
                                                                                this works well.
                                                                                if ($node->tag == "br") {
                                                                                ...
                                                                                HDOM_QUOTE_DOUBLE;
                                                                                $this->char = (++$this->pos < $this->
                                                                                    size) ? $this->doc[$this->pos] :
                                                                                    null; // next
                                                                                    $node->attr[$name] =
                                                                                    $this->restore_noise($this->copy_until_char_escape('"'));
                                                                                    $this->char = (++$this->pos <
                                                                                        $this->size) ?
                                                                                        $this->doc[$this->pos] : null;
                                                                                        // next
                                                                                        break;
                                                                                        case '\'':
                                                                                        $node->_[HDOM_INFO_QUOTE][] =
                                                                                        ...
                                                                                        $this->char = (++$this->pos <
                                                                                            ...
                                                                                            < $this->size) ?
                                                                                                ...
                                                                                                "class" selector, lets
                                                                                                get rid of the
                                                                                                preceeding and trailing
                                                                                                space since some people
                                                                                                leave it in the multi
                                                                                                class case.
                                                                                                if ($name == "class") {
                                                                                                $node->attr[$name] =
                                                                                                trim($node->attr[$name]);
                                                                ...
                                                                $this->char = ($this->pos < $this->size) ?
                                                                    ...
                                                                    $this->char = ($this->pos < $this->size) ?
                                                                        ...
                                                                        $this->char = ($this->pos < $this->size) ?
                                                                            ...
                                                                            $this->pos = $pos;
                                                                            return substr($this->doc, $pos_old, $pos -
                                                                            $pos_old);
                                                                            }
                                                                            ...
                                                                            $this->doc, $matches, PREG_SET_ORDER |
                                                                            PREG_OFFSET_CAPTURE);
                                                                            ...
                                                                            while (($pos = strpos($text, '___noise___'))
                                                                            !== false) {
                                                                            // Sometimes there is a broken piece of
                                                                            markup, and we don't GET the pos+11 etc...
                                                                            token which indicates a problem outside of
                                                                            us...
                                                                            ...
                                                                            if (is_object($debugObject)) {
                                                                            $debugObject->debugLog(2, 'located key of: '
                                                                            . $key);
                                                                            }

                                                                            if (isset($this->noise[$key])) {
                                                                            $text = substr($text, 0, $pos) .
                                                                            $this->noise[$key] . substr($text, $pos +
                                                                            16);
                                                                            } else {
                                                                            // do this to prevent an infinite loop.
                                                                            $text = substr($text, 0, $pos) . 'UNDEFINED
                                                                            NOISE FOR KEY: ' . $key . substr($text, $pos
                                                                            + 16);
                                                                            }
                                                                            } else {
                                                                            // There is no valid key being given back to
                                                                            us... We must get rid of the ___noise___ or
                                                                            we will have a problem.
                                                                            $text = substr($text, 0, $pos) . 'NO NUMERIC
                                                                            NOISE KEY' . substr($text, $pos + 11);
                                                                            }
                                                                            }
                                                                            return $text;
                                                                            }

                                                                            // Sometimes we NEED one of the noise
                                                                            elements.
